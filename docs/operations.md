...
`granite-rel-retry-grid` is centered on reliability work in Solidity. The technical target is: Develop a Solidity command-oriented project for retry scenarios with transition tables, invalid-transition tests, and no network dependency.
...
The score is deterministic. The current threshold is 170. Risk has a penalty of 7, latency has a penalty of 2, and weight contributes a bonus of 3.
